...
`main.m` builds the unit-disk geometry, assembles the coefficients, meshes the domain (`Hmax = 0.01`), solves for the steady-state velocity, interpolates the solution and its gradient onto a $100\times100$ Cartesian grid, and produces a six-panel figure. A run takes on the order of a minute on a laptop; increase `hmax` in `main.m` for a quicker, coarser solve.
...
| `compute_stress.m` | Evaluates the constitutive relation: strain-rate components $(e_1,e_2,e_3)\to$ viscous stress components $(\sigma_1,\sigma_2,\sigma_3)$ in the $\mathbf{D}_i$ basis. |
...
3. $\mathrm{Tr}[\nabla\mathbf{u}]/2 = e_1$: isotropic strain rate (expansion $>0$, compression $<0$).
